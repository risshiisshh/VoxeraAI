...
function TypingIndicator() {
  return (
    <div className="flex gap-2 items-center px-4 py-3">
      {[0,1,2].map((i) => (
        <span key={i} className="w-2 h-2 rounded-full bg-[#F5A623] animate-bounce" style={{ animationDelay:`${i*150}ms` }} />
      ))}
    </div>
  );
}
...
export default function AssistantPage() {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      setMessages((p) => p.map((m) => m.id===aiId ? {...m, content:"Sorry, something went wrong. Please try again.", loading:false} : m));
      ...
      <div className="flex h-[calc(100vh-4rem)] mt-16">
        ...
        <aside className="w-72 hidden lg:flex flex-col border-r border-white/[0.06] p-4 gap-4">
          ...
          <div className="p-3 rounded-xl bg-[#F5A623]/10 border border-[#F5A623]/20">
            ...
            <p className="text-xs text-[#8899BB] mt-0.5">{messages.length} messages</p>
          </div>
              ...
              user ? (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-[#4A5A7A] truncate">{user.email}</p>
                  </div>
                </div>
              ) : (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-[#4A5A7A] text-sm font-black">
                      👤
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-white truncate">Guest</p>
                      <p className="text-xs text-[#4A5A7A]">Progress not saved</p>
                    </div>
                  </div>
                  <button 
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex items-center gap-3 px-6 py-4 border-b border-white/[0.06] glass">
            <div className="w-9 h-9 rounded-full bg-[#F5A623] flex items-center justify-center text-[#0A1628] font-black">V</div>
            <div>
              <h1 className="font-bold text-white">VoxeraAI Assistant</h1>
              <div className="flex items-center gap-1.5 text-xs text-[#8899BB]"><span className="live-dot" />Powered by Gemini · Non-partisan</div>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4" role="log" aria-live="polite">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-3 animate-fade-up ${msg.role==="user"?"justify-end":"justify-start"}`}>
                {msg.role==="assistant" && <div className="w-8 h-8 rounded-full bg-[#F5A623]/20 flex items-center justify-center text-[#F5A623] text-xs font-black flex-shrink-0 mt-0.5">V</div>}
                <div className={`max-w-[72%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${msg.role==="user"?"bg-[#1D2E52] text-white rounded-tr-sm":"bg-[#162444] text-[#E0EAFF] rounded-tl-sm"}`}>
          ...
          {messages.length <= 2 && (
            <div className="px-6 pb-2 flex flex-wrap gap-2">
              {SUGGESTIONS.map((s) => <button key={s} className="badge badge-blue cursor-pointer text-xs" onClick={() => sendMessage(s)}>{s}</button>)}
            </div>
          )}
          <div className="px-6 pb-6 pt-2">
            ...
            <p className="text-xs text-[#4A5A7A] text-center mt-2">VoxeraAI may make mistakes. Verify at voters.eci.gov.in.</p>
          </div>
        </div>

        {/* Mini timeline */}
        <aside className="w-80 hidden xl:flex flex-col border-l border-white/[0.06] p-5 gap-4 overflow-y-auto">
          <h2 className="text-sm font-semibold text-[#8899BB] uppercase tracking-widest">Election Timeline</h2>
          {ELECTION_INFO.steps.map((step, i) => (
            <div key={step.id} className="flex items-start gap-3">
              <div className="flex flex-col items-center gap-1">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm ${step.status==="completed"?"bg-[#22C55E] text-white":step.status==="active"?"bg-[#F5A623] text-[#0A1628]":"border border-white/20 text-[#4A5A7A]"}`}>
                  {step.status==="completed"?"✓":step.icon}
                </div>
                {i<ELECTION_INFO.steps.length-1 && <div className={`w-0.5 h-6 ${step.status==="completed"?"bg-[#22C55E]":"bg-white/10"}`} />}
              </div>
              <div className="pb-2">
                <p className={`text-sm font-medium ${step.status==="active"?"text-[#F5A623]":"text-white"}`}>{step.title}</p>
                <p className="text-xs text-[#4A5A7A] mt-0.5">{step.date}</p>
              </div>
            </div>
          ))}
        </aside>
